...
namespace CipherShed
{
	...
	void UserInterface::CheckRequirementsForMountingVolume () const
	{
		...
		if (!Preferences.NonInteractive)
		{
			if (!SystemInfo::IsVersionAtLeast (2, 6, 24))
				ShowWarning (_("Your system uses an old version of the Linux kernel.\n\nDue to a bug in the Linux kernel, your system may stop responding when writing data to a CipherShed volume. This problem can be solved by upgrading the kernel to version 2.6.24 or later."));
		}
#endif // TC_LINUX
	}
...
#ifdef TC_LINUX
			if (string (volume.VirtualDevice).find ("/dev/mapper/ciphershed") != 0)
			{
#endif
	...
	wxString UserInterface::ExceptionToMessage (const exception &ex) const
	{
		...
		size_t pos = typeName.find ("CipherShed::");
		if (pos != string::npos)
		{
			return StringConverter::ToWide (typeName.substr (pos + string ("CipherShed::").size()))
...
#ifdef TC_MACOSX
		EX2MSG (HigherFuseVersionRequired,			_("CipherShed requires MacFUSE 1.3 or later."));
#endif
		...
		case CommandId::Help:
			{
				wstring helpText = StringConverter::ToWide (
					"Synopsis:\n"
					"\n"
					"ciphershed [OPTIONS] COMMAND\n"
					"ciphershed [OPTIONS] VOLUME_PATH [MOUNT_DIRECTORY]\n"
					"\n"
					"\n"
					"Commands:\n"
					"\n"
					...
					" 1) Path to the encrypted CipherShed volume.\n"
					...
					" command with option -o when a filesystem on a CipherShed volume is mounted.\n"
					...
					" Specifies comma-separated mount options for a CipherShed volume:\n"
					...
					"IMPORTANT:\n"
					"\n"
					"If you want to use CipherShed, you must follow the security requirements and\n"
					"security precautions listed in chapter 'Security Requirements and Precautions'\n"
					"in the CipherShed documentation (file 'CipherShed User Guide.pdf').\n"
					"\n"
					"\nExamples:\n\n"
					"Create a new volume:\n"
					"ciphershed -t -c\n"
					"\n"
					"Mount a volume:\n"
					"ciphershed volume.tc /media/ciphershed1\n"
					"\n"
					"Mount a volume as read-only, using keyfiles:\n"
					"ciphershed -m ro -k keyfile1,keyfile2 volume.tc\n"
					"\n"
					"Mount a volume without mounting its filesystem:\n"
					"ciphershed --filesystem=none volume.tc\n"
					"\n"
					"Mount a volume prompting only for its password:\n"
					"ciphershed -t -k \"\" --protect-hidden=no volume.tc /media/ciphershed1\n"
					"\n"
					"Dismount a volume:\n"
					"ciphershed -d volume.tc\n"
					"\n"
				...
				{
					wxDialog dialog (nullptr, wxID_ANY, _("CipherShed Command Line Help"), wxDefaultPosition);
